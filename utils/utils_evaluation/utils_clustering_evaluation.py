...
class VAE_clustering_evaluation():
    ...
    def plot_umap_orig(self, embedding, true_labels, dataset_name, model_type):
        mapper = umap.UMAP().fit(np.nan_to_num(np.array(embedding)))
        umap.plot.points(mapper, labels=true_labels, color_key_cmap='Paired', show_legend=True)
        title_orig = f'{model_type} (true labels)'
        plt.title(title_orig)
        plt.savefig(self.path_save_fig + f'/{model_type}_{dataset_name}_umap_original.png')
        plt.show()
    ...
    def plot_umap_cluster(self, embedding, clusters, dataset_name, model_type, ari, nmi):
        ...
        title_clusters = f'{model_type} ({self.clustering_method})\nARI: {ari}, NMI: {nmi}'
        plt.title(title_clusters)
        plt.savefig(self.path_save_fig + f'/{model_type}_{dataset_name}_umap_clusters.png')
        plt.show()
