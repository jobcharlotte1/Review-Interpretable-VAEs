...
def safe_filename(name: str) -> str:
    # Replace characters that are invalid in filenames (on any OS)
...
def proba_impact_pathway_perturbation(df_scores, overlap_matrix, pathway_observed, pathway_compared, name_reduction_metric, name_activation_column, name_overlap_compared_pathway, name_overlap_column, activ_threshold, overlap_threshold):
    ...
    observed_filter = (df_scores['pathway observed'] == pathway_observed) & (df_scores['pathway perturbated'] == pathway_observed)
    
    if observed_filter.sum() == 0:
        return None
    
    observed_value  = df_scores[observed_filter][name_reduction_metric].values[0]
...
def overall_proba_impact_pathway_perturbation(
    ...
    def run_one(pathway_compared):
        df_scores_subset = df_scores[df_scores['pathway perturbated'] == pathway_observed]
    ...
    if n_cpus == 1:
        results = [run_one(pathway_compared) for pathway_compared in list_pathways]
    else:
        
        results_raw = Parallel(n_jobs=n_cpus)(
...
def compute_metrics_for_pathway(
    ...
    if model_name == 'OntoVAE':
        ...
        file_path = os.path.join(
            path_to_save_reduction_scores,
            f'{model_name}_reduction_scores_{pathway_perturbated}_perturbation.parquet'
        )
        if not os.path.exists(file_path):
            print(f"pass: missing pathway : {file_path}")
            return pathway_perturbated, {}, []
        df_scores_all = pd.read_parquet(file_path)
        df_scores = df_scores_all[df_scores_all['pathway observed'].isin(list_pathways)]
