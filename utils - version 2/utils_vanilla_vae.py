...
class DynamicVanillaVAE(nn.Module):
    def __init__(self, n_genes, n_latent, encoder_layers=[800, 800], decoder_layers=[800, 800], **kwargs):
        super(DynamicVanillaVAE, self).__init__()
        ...
        self.encoder = self._build_mlp([self.n_genes] + self.encoder_layers, dropout=self.dropout)
        self.mean = nn.Linear(self.encoder_layers[-1], self.n_latent)
        self.logvar = nn.Linear(self.encoder_layers[-1], self.n_latent)

        self.decoder = self._build_mlp([self.n_latent] + self.decoder_layers + [self.n_genes], dropout=self.dropout, last_layer_no_bn=True)

        if self.init_w:
            self.apply(self._init_weights)

    def _build_mlp(self, layer_sizes, dropout=0.3, last_layer_no_bn=False):
        """Dynamically builds an MLP with BatchNorm, ReLU, and Dropout."""
        layers = []
        for i in range(len(layer_sizes) - 1):
            in_dim = layer_sizes[i]
            out_dim = layer_sizes[i + 1]
            layers.append(nn.Linear(in_dim, out_dim))
            if i != len(layer_sizes) - 2 or not last_layer_no_bn:
    ...
    def train_model(self, train_loader, learning_rate, n_epochs, train_patience, test_patience, test_loader=False, save_model=True):
        ...
        optimizer = optim.AdamW(self.parameters(), lr=learning_rate, weight_decay=1e-2)
