...
class DynamicVEGA(torch.nn.Module):
    def __init__(self, pathway_mask, positive_decoder=False, hidden_layers=[800, 800], **kwargs):
        ...
        self.mean = nn.Sequential(nn.Linear(hidden_layers[-1], self.n_pathways), nn.Dropout(self.dropout))
        self.logvar = nn.Sequential(nn.Linear(hidden_layers[-1], self.n_pathways), nn.Dropout(self.dropout))
